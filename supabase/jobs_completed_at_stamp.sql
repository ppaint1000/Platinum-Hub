-- Platinum Painters Hub — auto-stamp jobs.completed_at, same pattern as
-- won_at/quoted_at. Extends the existing assign_job_number() trigger
-- rather than adding a new one — it already runs before every
-- insert/update on jobs and already does the equivalent for those two.
-- Run once in the Supabase SQL Editor. Safe to re-run.

alter table public.jobs add column if not exists completed_at date;
